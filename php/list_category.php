...
[$consulta_datos, $consulta_total] = (isset($busqueda) && $busqueda != "") ?
    [
        "SELECT * FROM categoria WHERE (name LIKE '%$busqueda%' OR ubicacion LIKE '%$busqueda%') 
                     ORDER BY name ASC LIMIT $inicio,$registros"
        ,
        "SELECT COUNT(id) FROM categoria WHERE (name LIKE '%$busqueda%' OR ubicacion LIKE '%$busqueda%') "
    ]
    :
    [
        "SELECT * FROM categoria ORDER BY name ASC LIMIT $inicio,$registros"
        ,
    ...
    foreach ($datos as $rows) {
        $tabla .= '
				<tr class="has-text-centered" >
                    ...
                    <td>
                        <a href="index.php?view=category_update&user_id_up=' . $rows['id'] . '" class="button is-success is-rounded is-small">Actualizar</a>
                    </td>
                    <td>
